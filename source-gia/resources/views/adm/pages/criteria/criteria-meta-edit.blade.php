    ...
    <section class="content-header">
      ...
      <div class="col-sm-6">
            <ol class="breadcrumb ">
              ...
              <li class="breadcrumb-item active">Edit Assign Criteria</li>
            </ol>
          </div>
          ...
          <div class="card-body">
            ...
            <div class="col-md-12 card card-theme">
              ...
              <div class="card-header">
                <h3 class="card-title">Assign Criteria</h3>
              ...
              <form method="post" enctype="multipart/form-data"  class="form-horizontal"
               action="{{route('admin.criteria.meta.update')}}">
                @csrf

                <div class="card-body p-2 row">
                    <div class="col-sm-6">
                    <input type="hidden" class="form-control" name="id" value="{{$criteriaMeta->id}}">

                    <label for="criteria">Criteria</label>
                    <input class="form-control" type="text" name="criteria" value="{{$criteria->title}}" disabled>
                            ...
                            @if($categories)
                                @foreach($categories as $category)
                                    <option value="{{$category->id}}"
                                      <?php
                                         $categoryArrs = explode(',', $criteriaMeta->categories);
                                         foreach($categoryArrs as $categoryArr){
                                           if($categoryArr == $category->id)
                                           echo 'selected';
                                         }
                                      ?>
...
<!-- 
                        ...
                        <option value="">Assign Product</option>
                          ...
                          @foreach($products as $product)
                              <option value="{{$product->id}}"
                                  <?php
                                    $productArrs = explode(',', $criteriaMeta->products);
                                    foreach($productArrs as $productArr){
                                      if($productArr == $product->id)
                                      echo 'selected';
                                    }
                                ?>
                  ...
                  <div class="form-check mt-4">
                                ...
                                @if($criteriaMeta->status == 0)
                                <h5> <span class="badge badge-danger">Inactive</span></h5>@else<h5> <span class="badge badge-success">Active</span>
                                </h5>@endif</td>
                    </div>	
...
<!--                         
                        ...
                        <td>
                          ...
                          {{-- <a href="{{route('Criteria-image.edit',$criteriaMeta->id)}}" class="btn btn-xs btn-info float-left mr-2"  title="Upload Criteria Images"><i class="far fa-edit"></i></a> --}}
                          <button class="btn btn-xs btn-danger del-modal float-left"  title="Delete Criteria"  data-id="{{route('admin.index')}}/criteria/{{$criteriaMeta->id}}" 
                            data-image="{{url('web')}}/media/sm/{{ $criteriaMeta->image}}" data-title="{{ $criteriaMeta->title}}"  data-toggle="modal" data-target="#modal-default"><i class="fas fa-trash-alt"></i>
                          </button>
